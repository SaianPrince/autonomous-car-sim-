"""
brain.py — Week 4: AI Integration and Final
Protocol:
  C++ -> Python : 200x200 RGBA raw pixels = 160000 bytes
  Python -> C++ : angle string like "12.45\n" or "STOP\n"
"""
...
def detect_red_obstacle(bgr: np.ndarray):
    ...
    return obstacle_detected, object_debug, red_mask
...
def start_server() -> None:
    ...
    print(
        f"[BRAIN] Listening on {HOST}:{PORT}...",
        flush=True
    )
    ...
    try:
        while True:
            ...
            angle, lane_debug = detect_lanes(
                gray,
                pid,
                dt
            )

            obstacle_detected, object_debug, red_mask = detect_red_obstacle(
                bgr
            )
